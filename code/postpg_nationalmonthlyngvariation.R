...
fuel_rowone <- read_csv(paste0(RunFdr,'/Fuels_data.csv'), col_types = cols())[1,] # this row store the emission factor.
fuel_order = colnames(fuel_price)[-1]
reprepoint <- read_csv(paste0(RunFdr,'/Representative_Period.csv'), col_types = cols())
rep_hour <- select(read_csv(paste0(RunFdr,'/Load_data.csv'), col_types = cols()), Timesteps_per_Rep_Period)$Timesteps_per_Rep_Period[1]
...
fuel_price_variate <- fuel_price %>%
  ...
  left_join(mapping) %>%
  pivot_longer(cols = -c(Time_Index, Period, Period_Hour, Month), names_to = 'fuel', values_to = 'price') %>%
...
write_csv(fuel_price, paste0(RunFdr,'/Fuels_data_original.csv'))
